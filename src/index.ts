...
const WORKTREE_REVIEW_PROMPT_WITH_MERGE_BASE =
  "Review the code changes for {label} against the base branch '{baseBranch}'. The code is checked out in a separate git worktree at {worktreePath} — review THAT worktree, not the current working directory. The merge base commit for this comparison is {mergeBaseSha}. Run `git -C {worktreePath} diff {mergeBaseSha} HEAD` to inspect the changes (committed changes only — ignore any uncommitted working-tree state), and read files under {worktreePath} for full context. Provide prioritized, actionable findings.";

const WORKTREE_REVIEW_PROMPT_FALLBACK =
  "Review the code changes for {label} against the base branch '{baseBranch}'. The code is checked out in a separate git worktree at {worktreePath} — review THAT worktree, not the current working directory. Find the merge base with `git -C {worktreePath} merge-base {baseBranch} HEAD`, then run `git -C {worktreePath} diff <merge-base> HEAD` to inspect the changes (committed changes only — ignore any uncommitted working-tree state). Provide prioritized, actionable findings.";
